...
| Key | Value |
|-----|--------|
| `DATABASE_URL` | `postgresql+asyncpg://sumaya:sumaya_secret@NGROK_HOST:NGROK_PORT/sumaya_resto` |
| `CORS_ORIGINS` | `https://sumaya-web.onrender.com` (your actual web URL) |
| `SECRET_KEY` | Auto-generated by Render (or set a long random string) |
| `REDIS_URL` | Optional — leave empty if not using Redis yet |

**sumaya-web** (build-time)

| Key | Value |
|-----|--------|
| `VITE_API_URL` | `https://sumaya-api.onrender.com/api/v1` (your actual API URL) |

4. Deploy both services.
5. After API is live, trigger a **manual deploy** of `sumaya-web` if env vars were set after first build.
